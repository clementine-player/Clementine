...
TcpRemoteClient::TcpRemoteClient(
    Application* app, QTcpSocket* client, QObject* parent)
...
void TcpRemoteClient::IncomingData() {
...
void TcpRemoteClient::ParseMessage(const QByteArray& data) {
  ...
  if (msg.type() == pb::remote::CONNECT) {
    set_downloader(msg.request_connect().downloader());
...
void TcpRemoteClient::DisconnectClient(pb::remote::ReasonDisconnect reason) {
  pb::remote::Message msg;
...
void TcpRemoteClient::SendDataToClient(pb::remote::Message* msg) {
...
void TcpRemoteClient::SendData(pb::remote::Message* msg) {
...
QAbstractSocket::SocketState TcpRemoteClient::state() { return client_->state(); }
